...
#ifndef ARROW_DECIMAL_NATIVE_H
#define ARROW_DECIMAL_NATIVE_H
...
class ARROW_EXPORT Decimal128 {
 public:
  ...
  constexpr Decimal128(int64_t high, uint64_t low)
      : value_((static_cast<__int128>(high) << 64) | static_cast<__int128>(low)) {}
  ...
  constexpr Decimal128(T value)
      : Decimal128(value >= 0 ? 0 : -1, static_cast<uint64_t>(value)) {}

  constexpr Decimal128(__int128 value) : Decimal128(value >> 64, value << 64) {}
  ...
  explicit operator char() const { return static_cast<char>(value_); }
  ...
  bool operator==(const Decimal128& right) const { return value_ == right.value_; }
  ...
  Decimal128 operator~() const { return ~value_; }

  /// \brief Get the high bits of the two's complement representation of the number.
  int64_t high_bits() const { return static_cast<int64_t>(value_ >> 64); }

  /// \brief Get the low bits of the two's complement representation of the number.
  uint64_t low_bits() const { return static_cast<uint64_t>(value_ & 0xFFFFFFFFFFFFFFFF); }

  /// \brief Return the raw bytes of the value in little-endian byte order.
  std::array<uint8_t, 16> ToBytes() const {
    __int128 value = BitUtil::ToLittleEndian(value_);
    auto raw_bytes = reinterpret_cast<const uint8_t*>(&value);
    std::array<uint8_t, 16> result{{0}};
    std::copy(raw_bytes, raw_bytes + result.size(), result.begin());
    return result;
  }
